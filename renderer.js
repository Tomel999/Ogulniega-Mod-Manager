document.addEventListener("DOMContentLoaded", () => {
  ...
  const activeMcVersionsDisplay = document.getElementById(
    "activeMcVersionsDisplay"
  );

  const initialVersionSelectionModal = document.getElementById(
    "initialVersionSelectionModal"
  );
  const mcVersionSelect = document.getElementById("mcVersionSelect");
  const confirmInitialVersionButton = document.getElementById(
    "confirmInitialVersionButton"
  );
  const openVersionSelectionModalButton = document.getElementById(
    "openVersionSelectionModalButton"
  );
  ...
  const closeVersionModalButton = document.getElementById(
    "closeVersionModalButton"
  );
  ...
  const openSelectedFolderButton = document.getElementById(
  ...
  const closeFolderModalButton = document.getElementById(
    "closeFolderModalButton"
  );

  const preinstalledProfileSelectionModal = document.getElementById(
    "preinstalledProfileSelectionModal"
  );
  const preinstalledModalMcVersion = document.getElementById(
    "preinstalledModalMcVersion"
  );
  const preinstalledProfileSelect = document.getElementById(
    "preinstalledProfileSelect"
  );
  const confirmPreinstalledProfileButton = document.getElementById(
    "confirmPreinstalledProfileButton"
  );
  const cancelPreinstalledProfileButton = document.getElementById(
    "cancelPreinstalledProfileButton"
  );
  const closePreinstalledProfileModalButton = document.getElementById(
    "closePreinstalledProfileModalButton"
  );

  const defaultModsFilterContainer = document.getElementById(
    "defaultModsFilterContainer"
  );
  const defaultModFilterQuery = document.getElementById(
    "defaultModFilterQuery"
  );
  ...
  function updateUIState() {
    ...
    if (!isMcVersionSelected) {
      ...
      showStatus(
        "Proszę wybrać wersję Minecrafta, aby rozpocząć wyszukiwanie.",
        "info"
      );
  ...
  async function loadMinecraftVersions() {
    try {
      ...
      if (availableMcVersions.length > 0) {
        ...
        mcVersionSelect.innerHTML =
          '<option value="">-- Brak znalezionych wersji --</option>';
        mcVersionSelect.disabled = true;
        confirmInitialVersionButton.disabled = true;
        showStatus(
          "Nie znaleziono żadnych wersji Minecrafta w folderach profili. Proszę zainstalować wersje.",
          "error"
        );
      }
      ...
      console.error(
        "Błąd podczas wczytywania wersji Minecrafta z profili:",
        error
      );
      activeMcVersionsDisplay.textContent = "Błąd wczytywania";
      mcVersionSelect.innerHTML =
        '<option value="">-- Błąd wczytywania wersji --</option>';
      mcVersionSelect.disabled = true;
      confirmInitialVersionButton.disabled = true;
      window.electronAPI.showErrorMessage({
        title: "Błąd Wczytywania Wersji MC",
        content:
          "Nie udało się wczytać dostępnych wersji Minecrafta z folderów profili.",
      });
    } finally {
      updateUIState();
    }
  ...
  function updateActiveFiltersDisplay() {
    ...
    activeLoaderDisplay.textContent =
      selectedLoader.charAt(0).toUpperCase() + selectedLoader.slice(1);

    let versionsText;
    if (mcVersionSelect.value === "") {
      versionsText =
        availableMcVersions.length > 0 ? availableMcVersions[0] : "N/A";
    } else {
  ...
  mcVersionSelect.addEventListener("change", () => {
    if (mcVersionSelect.value !== "") {
      ...
      confirmInitialVersionButton.disabled = true;
      activeMcVersionsDisplay.textContent =
        availableMcVersions.length > 0 ? availableMcVersions[0] : "N/A";
    }
  });
  ...
  async function performSearch() {
    ...
    if (currentMcVersions.length === 0 || mcVersionSelect.value === "") {
      showStatus(
        "Proszę wybrać wersję Minecrafta, aby wyszukać mody.",
        "warning"
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `${MODRINTH_API_BASE}/search?query=${encodeURIComponent(
          query
        )}&facets=${encodeURIComponent(JSON.stringify(facets))}&limit=20`,
        {
          headers: {
            "User-Agent": "ElectronPureJSModrinthApp/1.9.9 (DefaultModFilter)",
          },
        }
      );
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ description: "Nieznany błąd API." }));
        throw new Error(
          `Błąd API Modrinth: ${response.status} - ${
      ...
      showStatus(
        `Znaleziono ${data.hits.length} modów pasujących do kryteriów.`,
        "success"
      );
    } catch (error) {
  ...
  function displaySearchResultsAsCards(hits) {
    ...
    hits.forEach((mod, index) => {
      ...
      const modGameVersions = Array.isArray(mod.game_versions)
        ? mod.game_versions
        : [];
      const versionsDisplay =
        modGameVersions.length > 0
          ? modGameVersions.join(", ")
          : "Brak info o wersjach";
      const categoriesDisplay = Array.isArray(mod.categories)
        ? mod.categories.map((c) => c.replace(/_/g, " ")).join(", ")
        : "Brak kategorii";

      modCard.innerHTML = `
                <div class="mod-card-header">
                    <img src="${mod.icon_url || "icons/jar.png"}" alt="${
        mod.title || "Mod Icon"
      }" class="mod-icon" onerror="this.onerror=null;this.src='icons/jar.png';">
                ...
                <div class="mod-card-actions">
                    <button class="btn btn-primary download-mod-button" data-mod-id="${
                      mod.project_id
                    }" data-mod-slug="${mod.slug}" data-mod-title="${
        mod.title || "Mod"
      }">
                        Pobierz / Wersje
                    </button>
  ...
  async function handleModVersionSelection(event) {
    const modId =
      event.currentTarget.dataset.modId || event.currentTarget.dataset.modSlug;
    ...
    if (!modId) {
      window.electronAPI.showErrorMessage({
        title: "Błąd",
        content: "Nie można zidentyfikować moda.",
      });
      return;
    }
    ...
    try {
      ...
      if (
        !response.ok ||
        (response.ok && (!versionsData || versionsData.length === 0))
      ) {
        ...
        if (response.ok && (!versionsData || versionsData.length === 0)) {
          showStatus(
            `Brak wersji dla "${modTitle}" z filtrami ${filterStatusText}, pobieranie wszystkich...`,
            "warning"
          );
        } else if (!response.ok && response.status !== 404) {
          const errorData = await response.json().catch(() => ({}));
          console.warn(
            `Błąd API (wersje z filtrem ${
              response.status
            } ${filterStatusText}): ${
              errorData.description || response.statusText
            }. Próba pobrania wszystkich wersji.`
          );
        }
        console.log(`Pobieranie wszystkich wersji z: ${allVersionsUrl}`);
        response = await fetch(allVersionsUrl, {
          headers: { "User-Agent": "ElectronPureJSModrinthApp/1.9.9" },
        });
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(
            `Błąd API (wszystkie wersje ${response.status}): ${
              errorData.description || response.statusText
            }`
          );
        }
      ...
      if (!versionsData || versionsData.length === 0) {
        modalVersionsList.innerHTML =
          '<p class="no-results">Nie znaleziono żadnych wersji dla tego moda.</p>';
        showStatus(
          `Nie znaleziono żadnych wersji dla "${modTitle}".`,
          "warning"
        );
        return;
      }
      ...
      showStatus(
        `Błąd pobierania wersji dla "${modTitle}": ${error.message}`,
        "error"
      );
      window.electronAPI.showErrorMessage({
        title: "Błąd Pobierania Wersji",
        content: error.message,
      });
    }
  }
  ...
  function displayVersionsInModal(versions, modTitle, targetMcVersions) {
    ...
    if (!Array.isArray(versions)) {
      modalVersionsList.innerHTML =
        '<p class="no-results">Otrzymano nieprawidłowe dane wersji.</p>';
      return;
    }
    ...
    if (filteredVersions.length === 0 && versions.length > 0) {
      ...
      warningMessage = `<p class="warning-text">Brak wersji idealnie pasujących do MC ${targetMcVersions.join(
        ", "
      )}. Wyświetlanie wszystkich dostępnych wersji dla tego moda.</p>`;
    } else if (versionsToDisplay.length === 0) {
      modalVersionsList.innerHTML =
        '<p class="no-results">Brak wersji moda zgodnych z wybraną wersją Minecrafta.</p>';
      return;
    }
    ...
    versionsToDisplay
      ...
      .forEach((version, index) => {
        ...
        const datePublished = version.date_published
          ? new Date(version.date_published).toLocaleDateString("pl-PL")
          : "Brak daty";
        const fileSizeMB = primaryFile.size
          ? (primaryFile.size / 1024 / 1024).toFixed(2)
          : "N/A";

        const isPerfectMatch =
          Array.isArray(version.game_versions) &&
          version.game_versions.some((gv) => targetMcVersions.includes(gv));
        const isFirstPerfectMatch =
          isPerfectMatch &&
          filteredVersions.findIndex((fv) => fv.id === version.id) === 0;
        const recommendedLabel = isFirstPerfectMatch
          ? '<div class="recommended-label">Zalecana dla wybranej wersji MC</div>'
          : "";
        const buttonClass = isFirstPerfectMatch
          ? "btn-download-version-top"
          : "btn-download-version-secondary";

        item.innerHTML = `
                <div class="version-details">
                    <p class="version-name">${version.name || "N/A"} (${
          version.version_number || "N/A"
        })</p>
                    <p>MC: ${
                      Array.isArray(version.game_versions)
    ...
    if (!initialPathToSetInInput) {
      ...
      if (profileData.baseProfilePath && selectedMcVersion) {
        initialPathToSetInInput = `${profileData.baseProfilePath}${
          profileData.baseProfilePath.includes("/") ? "/" : "\\"
        }${selectedMcVersion}`;
  ...
  browseFolderButton.addEventListener("click", async () => {
    try {
      ...
      window.electronAPI.showErrorMessage({
        title: "Błąd Przeglądania",
        content: "Nie udało się otworzyć okna wyboru folderu.",
      });
    }
  });
  ...
  openSelectedFolderButton.addEventListener("click", () => {
    ...
    if (targetPath) {
      console.log(
        "[RendererJS] Próba otwarcia folderu (z przycisku w modalu):",
        targetPath
      );
      ...
      window.electronAPI.showErrorMessage({
        title: "Brak ścieżki",
        content:
          "Proszę najpierw wybrać profil lub wprowadzić ścieżkę do folderu.",
      });
    }
  });
  ...
  confirmFolderButton.addEventListener("click", () => {
    ...
    if (!selectedDirectory) {
      window.electronAPI.showErrorMessage({
        title: "Brak folderu",
        content: "Proszę wybrać lub wprowadzić folder docelowy.",
      });
      return;
    }
    if (!currentDownloadInfo) {
      window.electronAPI.showErrorMessage({
        title: "Błąd",
        content: "Brak informacji o pliku do pobrania.",
      });
    ...
    folderSelectionModal.classList.remove("active");
    showStatus(
      `Pobieranie "${currentDownloadInfo.filename}" do "${selectedDirectory}"...`,
      "info"
    );
    ...
    window.electronAPI
      ...
      .catch((error) => {
        ...
        showStatus(
          `Błąd inicjacji pobierania: ${error.message || error}`,
          "error"
        );
        progressBarContainer.style.display = "none";
        window.electronAPI.showErrorMessage({
          title: "Błąd Pobierania",
          content: `Nie udało się rozpocząć pobierania pliku "${
            currentDownloadInfo.filename
  ...
  function displayPreinstalledMods(filesToDisplay) {
    ...
    if (!filesToDisplay || filesToDisplay.length === 0) {
      ...
      if (filterIsActive) {
        resultsArea.innerHTML =
          '<p class="no-results">Nie znaleziono domyślnych modów pasujących do Twojego filtra.</p>';
      } else {
        resultsArea.innerHTML =
          '<p class="no-results">Nie znaleziono plików .jar w folderze "preinstalled" dla tego profilu.</p>';
      }
      return;
    }
    ...
    filesToDisplay.forEach((file, index) => {
      ...
      const buttonText = isEnabled ? "Włączony" : "Wyłączony";
      const buttonActionClass = isEnabled ? "btn-primary" : "btn-danger-state";
      ...
      modCard.innerHTML = `
                <div class="mod-card-header">
                    ...
                    <div class="mod-title-author">
                        ...
                        <p class="mod-status-text">Plik lokalny (${
                ...
                <div class="mod-card-body">
                    ...
                    <div class="mod-card-info">
                        <p>Ścieżka na dysku: <strong class="mod-relative-path">${
                          file.relativePath
                        }</strong></p>
                    </div>
                </div>
                <div class="mod-card-actions">
                    <button class="btn ${buttonActionClass} toggle-mod-button" data-current-fullpath="${
        file.path
      }" data-card-id="local-mod-card-${index}">
                        ${buttonText}
                    </button>
                </div>`;
      resultsArea.appendChild(modCard);
    ...
    document.querySelectorAll(".toggle-mod-button").forEach((button) => {
      button.addEventListener("click", async (event) => {
        ...
        if (!currentFullPath || !modCardElement) {
          console.error(
            "Błąd: Brak ścieżki pliku lub elementu karty dla operacji toggle."
          );
          window.electronAPI.showErrorMessage({
            title: "Błąd wewnętrzny",
            content: "Nie można zmienić stanu moda, brak referencji do pliku.",
          });
          return;
        }
        ...
        try {
          const result = await window.electronAPI.toggleModState(
            currentFullPath
          );
          if (result.success) {
            ...
            modCardElement.querySelector(".mod-filename").textContent =
              originalFileObject.displayName;
            modCardElement.querySelector(
              ".mod-status-text"
            ).textContent = `Plik lokalny (${
              isNowEnabled ? "Włączony" : "Wyłączony"
            })`;
            modCardElement.querySelector(".mod-relative-path").textContent =
              originalFileObject.relativePath;

            const toggleButton =
              modCardElement.querySelector(".toggle-mod-button");
            toggleButton.textContent = isNowEnabled ? "Włączony" : "Wyłączony";
            toggleButton.classList.remove("btn-danger-state", "btn-primary");
            toggleButton.classList.add(
              isNowEnabled ? "btn-primary" : "btn-danger-state"
            );
            toggleButton.dataset.currentFullpath = result.newFullPath;

            showStatus(
              `Mod "${originalFileObject.displayName}" został ${
                isNowEnabled ? "pokazany (aktywny)" : "ukryty (nieaktywny)"
              }.`,
              "success"
            );
          } else {
            throw new Error(
              result.error || "Nieznany błąd podczas przełączania stanu moda."
            );
          }
        } catch (error) {
          ...
          window.electronAPI.showErrorMessage({
            title: "Błąd Operacji",
            content: `Nie udało się przełączyć stanu moda: ${error.message}`,
          });
  ...
  async function fetchAndDisplayPreinstalledMods(profilePath, mcVersion) {
    const profileName = profilePath
      .split(profilePath.includes("/") ? "/" : "\\")
      .pop();

    showStatus(
      `Wczytywanie domyślnych modów dla profilu '${profileName}' (folder 'preinstalled')...`,
      "info",
      true
    );
    resultsArea.innerHTML = '<div class="loader"></div>';
    ...
    try {
      ...
      if (result.success) {
        ...
        if (displayedPreinstalledModsFullList.length > 0) {
          showStatus(
            `Znaleziono ${displayedPreinstalledModsFullList.length} domyślnych modów w profilu '${profileName}'.`,
            "success"
          );
        } else {
          showStatus(
            `Nie znaleziono domyślnych modów w folderze "preinstalled" profilu '${profileName}'.`,
            "info"
          );
        }
      } else {
        throw new Error(
          result.error || "Nieznany błąd podczas wczytywania domyślnych modów."
        );
      }
    } catch (error) {
      ...
      defaultModsFilterContainer.style.display = "none";
      showStatus(
        `Błąd wczytywania domyślnych modów dla profilu '${profileName}': ${error.message}`,
        "error"
      );
      window.electronAPI.showErrorMessage({
        title: "Błąd Domyślnych Modów",
        content: `Nie udało się wczytać domyślnych modów. ${error.message}`,
      });
    }
  }

  function handleProfileSelectionForPreinstalled(
    profilesForVersion,
    mcVersion
  ) {
  ...
  defaultModsButton.addEventListener("click", async () => {
    ...
    if (!selectedMcVersion) {
      ...
      window.electronAPI.showErrorMessage({
        title: "Brak wersji MC",
        content:
          "Aby wyświetlić domyślne mody, najpierw wybierz wersję Minecrafta.",
      });
      return;
    }
    ...
    if (!profilesForVersion || profilesForVersion.length === 0) {
      console.warn(
        `Brak informacji o profilu dla ${selectedMcVersion} w groupedProfileFolders. Próba ponownego załadowania.`
      );
      ...
      if (!profilesForVersion || profilesForVersion.length === 0) {
        showStatus(
          `Nie można ustalić ścieżki profilu dla wersji ${selectedMcVersion}. Sprawdź konfigurację profili.`,
          "error"
        );
        window.electronAPI.showErrorMessage({
          title: "Błąd Ścieżki Profilu",
          content: `Nie znaleziono skonfigurowanej ścieżki profilu dla wersji Minecraft ${selectedMcVersion}. Upewnij się, że profile są poprawnie załadowane.`,
        });
        return;
      }
    }

    handleProfileSelectionForPreinstalled(
      profilesForVersion,
      selectedMcVersion
    );
  });
  ...
  confirmPreinstalledProfileButton.addEventListener("click", () => {
    ...
    if (
      selectedPath &&
      typeof afterProfileSelectedForPreinstalledCallback === "function"
    ) {
  ...
  window.electronAPI.onDownloadProgress(
    ({ filename, receivedBytes, totalBytes }) => {
      ...
      if (progressBarContainer.style.display === "flex") {
        statusText.textContent = `Pobieranie ${filename}: ${percent}% (${(
          receivedBytes /
          1024 /
          1024
        ).toFixed(2)}MB / ${
          totalBytes > 0 ? (totalBytes / 1024 / 1024).toFixed(2) : "N/A"
        }MB)`;
  ...
  window.electronAPI.onDownloadComplete(({ filename, path }) => {
    ...
    window.electronAPI.showInfoMessage({
      ...
      content: `Pomyślnie pobrano "${filename}" do:\n${path}`,
    });
  });
  ...
  window.electronAPI.onDownloadError(({ filename, error }) => {
    ...
    window.electronAPI.showErrorMessage({
      title: "Błąd Pobierania",
      content: `Nie udało się pobrać "${filename}".\nBłąd: ${error}`,
    });
  });
